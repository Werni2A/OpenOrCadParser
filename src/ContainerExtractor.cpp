    ...
    mReader->EnumFiles(mReader->GetRootEntry(), -1,
        [&, this](const CFB::COMPOUND_FILE_ENTRY* entry, const CFB::utf16string& /* dir */, int /* level */) -> void
        {
            if(mReader->GetEntry(entry->childID) != nullptr)
            {
                ...
                if(std::any_of(siblings.begin(), siblings.end(),
                       [&](const CFB::COMPOUND_FILE_ENTRY* sibling) { return aChild == sibling; }))
                {
                    parent = entry;
                }
            }
    ...
    mReader->EnumFiles(mReader->GetRootEntry(), -1,
        [&, this](const CFB::COMPOUND_FILE_ENTRY* entry, const CFB::utf16string& /* dir */, int /* level */) -> void
        {
            ...
            if(!isDirectory(entry))
            {
                ...
                {
                    // @todo OrCAD is able to read those files. Maybe the filesize is corrupted but OrCAD just parses
                    // them and stops when it thinks that the stream ended. But maybe there is some different
                    // information hidden in the size value?
