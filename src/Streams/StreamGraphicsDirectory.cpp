...
void StreamGraphicsDirectory::read(FileFormatVersion /* aVersion */)
{
    auto& ds = mCtx.mDs;
    ...
    if(size > 0U)
    {
        // mCtx.mLogger.warn("{} contains Graphic. Check it out!", mInputCfbfFile.string());
    }
    ...
    for(size_t i = 0u; i < size; ++i)
    {
        ...
        if(item.componentType != ComponentType::Graphic)
        {
