...
void StreamSymbolsDirectory::read(FileFormatVersion /* aVersion */)
{
    auto& ds = mCtx.mDs;
    ...
    for(size_t i = 0u; i < size; ++i)
    {
        ...
        std::vector<ComponentType> expected_types = {
            ComponentType::GlobalSymbol,
            ComponentType::PortSymbol,
            ComponentType::OffPageSymbol,
            ComponentType::TitleBlockSymbol,
            ComponentType::ERCSymbol,
            ComponentType::BookMarkSymbol
        };

        const auto result_it = std::find(expected_types.cbegin(), expected_types.cend(), item.componentType);
        if(result_it == expected_types.cend())
        {
